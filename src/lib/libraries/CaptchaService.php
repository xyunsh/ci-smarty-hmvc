...
class CaptchaService implements CaptchaServiceInterface {
    ...
    public function set() {
        ...
        $this->session->set_userdata('captcha', $captcha->getCaptcha());
    }
    ...
    public function check($auth_code) {
        $code = $this->get();
        if (empty($code) || empty($auth_code)) {
            return false;
        }

        return strtolower($code) == strtolower($auth_code);
        //return true;
    }
}
